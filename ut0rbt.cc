...
static ibool rbt_check_ordering(const ib_rbt_t* tree)
{
	...
	for(node = rbt_first(tree); node != NULL; node = rbt_next(tree, prev)){
		if(prev){
			int result;
...
UNIV_INTERN void rbt_free(ib_rbt_t*	tree)			/*!< in: rb tree to free */
{
...
UNIV_INTERN ib_rbt_t* rbt_create_arg_cmp(
/*===============*/
...
@return	an empty rb tree */
UNIV_INTERN ib_rbt_t* rbt_create(
	size_t		sizeof_value,		/*!< in: sizeof data item */
...
UNIV_INTERN const ib_rbt_node_t* rbt_insert(
	ib_rbt_t*	tree,			/*!< in: rb tree */
...
UNIV_INTERN const ib_rbt_node_t* rbt_add_node(
	ib_rbt_t*	tree,			/*!< in: rb tree */
...
UNIV_INTERN const ib_rbt_node_t* rbt_lookup(
	const ib_rbt_t*	tree,			/*!< in: rb tree */
...
UNIV_INTERN ib_rbt_node_t* rbt_remove_node(
	ib_rbt_t*		tree,		/*!< in: rb tree */
	const ib_rbt_node_t*	const_node)	/*!< in: node to delete, this is a fudge and declared const because the caller can access only const nodes */
{
	/* Cast away the const. */
...
UNIV_INTERN const ib_rbt_node_t* rbt_lower_bound(
	const ib_rbt_t*	tree,			/*!< in: rb tree */
...
UNIV_INTERN const ib_rbt_node_t* rbt_upper_bound(
/*============*/
	const ib_rbt_t*	tree,			/*!< in: rb tree */
	const void*	key)			/*!< in: key to search */
{
...
@return	value of result */
UNIV_INTERN int rbt_search_cmp(
	const ib_rbt_t*	tree,			/*!< in: rb tree */
	ib_rbt_bound_t*	parent,			/*!< in: search bounds */
	const void*	key,			/*!< in: key to search */
	ib_rbt_compare	compare,		/*!< in: fn to compare items */
	ib_rbt_arg_compare arg_compare)		/*!< in: fn to compare items with argument */
{
	ib_rbt_node_t*	current = ROOT(tree);
...
@return	no. of recs merged */
UNIV_INTERN ulint rbt_merge_uniq_destructive(
	ib_rbt_t*	dst,			/*!< in: dst rb tree */
